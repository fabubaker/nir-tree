...
TEST_CASE("R*TreeDisk: pack simple leaf node") {
    ...
    REQUIRE( root_leaf_node->compute_packed_size() <
            sizeof(LeafNodeType) );
    REQUIRE( root_leaf_node->compute_packed_size() == sizeof(unsigned) + sizeof(Point) * 5 );
...
TEST_CASE("R*TreeDisk: pack branch structure") {
    ...
    REQUIRE( root_node->compute_packed_size() == sizeof(unsigned) + sizeof(Branch) * 5 );
    ...
    REQUIRE( * (unsigned *) (buffer) == root_node->cur_offset_ );

    for( unsigned i = 0; i < 5; i++ ) {
        Branch *b = (Branch *) (buffer + (sizeof(unsigned)+ sizeof(Branch)*i));
...
TEST_CASE("R*TreeDisk: pack branch subtree") {
    ...
    for( unsigned i = 0; i < 5; i++ ) {
        ...
        REQUIRE( * (unsigned *) (buffer) == 7 );
        for( unsigned j = 0; j < 7; j++ ) {
            REQUIRE( * (Point *) (buffer + sizeof(unsigned)) ==
                Point(i,i));
        }
    }
