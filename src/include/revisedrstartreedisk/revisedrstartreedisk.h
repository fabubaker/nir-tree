...
#include <revisedrstartreedisk/node.h>
...
namespace revisedrstartreedisk {
    ...
    public:
        ...
        // RR* doesn't need this, but we keep it for a unified interface
        std::vector<bool> hasReinsertedOnLevel;

        // Constructors and destructors
        RevisedRStarTreeDisk(size_t memory_budget, std::string backing_file) : backing_file_(backing_file) {
    ...
    template <int min_branch_factor, int max_branch_factor>
    std::vector<Point> RevisedRStarTreeDisk<min_branch_factor, max_branch_factor>::exhaustiveSearch(Point requestedPoint) {
      std::vector<Point> v;
    ...
    template <int min_branch_factor, int max_branch_factor>
    std::vector<Point> RevisedRStarTreeDisk<min_branch_factor, max_branch_factor>::search(Point requestedPoint) {
      return point_search(root, requestedPoint, this);
    }

    template <int min_branch_factor, int max_branch_factor>
    std::vector<Point> RevisedRStarTreeDisk<min_branch_factor, max_branch_factor>::search(Rectangle
                                                                                   requestedRectangle) {
      return rectangle_search(root, requestedRectangle, this);
    }

    template <int min_branch_factor, int max_branch_factor>
    void RevisedRStarTreeDisk<min_branch_factor, max_branch_factor>::insert(Point givenPoint) {
      if (root.get_type() == LEAF_NODE) {
        auto root_ptr = get_leaf_node(root);
    ...
    template <int min_branch_factor, int max_branch_factor>
    void RevisedRStarTreeDisk<min_branch_factor, max_branch_factor>::remove(Point givenPoint) {
      if (root.get_type() == LEAF_NODE) {
        auto root_ptr = get_leaf_node(root);
    ...
    template <int min_branch_factor, int max_branch_factor>
    unsigned RevisedRStarTreeDisk<min_branch_factor, max_branch_factor>::checksum() {
      if (root.get_type() == LEAF_NODE) {
        auto root_ptr = get_leaf_node(root);
    ...
    template <int min_branch_factor, int max_branch_factor>
    void RevisedRStarTreeDisk<min_branch_factor, max_branch_factor>::print() {
      std::ofstream outputFile("printed_revised_rstar_tree.txt");

      struct Printer {
          Printer(std::ofstream &printFile): printFile(printFile) {}

          void operator()(RevisedRStarTreeDisk<min_branch_factor, max_branch_factor> *treeRef, tree_node_handle node_handle) {
    ...
    template <int min_branch_factor, int max_branch_factor>
    bool RevisedRStarTreeDisk<min_branch_factor, max_branch_factor>::validate() {
      return true;
    }

    template <int min_branch_factor, int max_branch_factor>
    void RevisedRStarTreeDisk<min_branch_factor, max_branch_factor>::stat() {
      stat_node(root, this);
    }

    template <int min_branch_factor, int max_branch_factor>
    void RevisedRStarTreeDisk<min_branch_factor, max_branch_factor>::visualize() {
