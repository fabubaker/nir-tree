...
public:
  ...
  // removeBranch: just remove branch from Node without deleting the branch
  void removeBranch(const Branch &entry);
...
Partition LeafNode<min_branch_factor, max_branch_factor>::partitionLeafNode() {
  ...
  //   defaultPartition.location = Point::closest_smaller_point(minBranchPoint_right)[choosenDimension];
  // } else {
  //   defaultPartition.location = average[choosenDimension];
  // }
...
SplitResult LeafNode<min_branch_factor, max_branch_factor>::splitNode(
  ...
  // if (this->cur_offset_ < min_branch_factor){
  //   std::cout << "left is smaller than min_branch_factor: " << this->cur_offset_ << std::endl;
  // }
  // if (sibling_node->cur_offset_ < min_branch_factor) {
  //   std::cout << "right is smaller than min_branch_factor: " << sibling_node->cur_offset_ << std::endl;
  // }
        ...
        std::vector<bool> &hasReinsertedOnLevel)
{
  ...
  while (current_handle != nullptr) {
    ...
    // Notes: 
    // With good balanced partition strategy, we would expect to update left branch and add right branch. 
    // Treat with unfull nodes
    ...
    if (propagationSplit.leftBranch.child.get_type() == LEAF_NODE) {
      ...
      if (left_node->cur_offset_ == 0) {
        ...
        if (current_handle == nullptr) {
          allocator->free(propagationSplit.leftBranch.child, sizeof(LeafNode<min_branch_factor, max_branch_factor>));
        } else {
        ...
        if (current_handle != nullptr) {
          auto parent_node = treeRef->get_branch_node(current_handle);
          parent_node->updateBranch(propagationSplit.leftBranch);
        }
      }
    ...
    } else if (propagationSplit.leftBranch.child.get_type() == BRANCH_NODE) {
      auto left_node = treeRef->get_branch_node(propagationSplit.leftBranch.child);
      if (left_node->cur_offset_ == 0) {
        // remove node
        if (current_handle == nullptr) {
        ...
        std::vector<bool> &hasReinsertedOnLevel)
{
  ...
  } else if (finalSplit.leftBranch.child == nullptr and finalSplit.rightBranch.child != nullptr) {
    // update root in tree 
    treeRef->root = finalSplit.rightBranch.child;
    return treeRef->root;
  }
...
template <int min_branch_factor, int max_branch_factor>
void BranchNode<min_branch_factor, max_branch_factor>::removeBranch(const Branch &entry)
{
  // Locate the child
  unsigned childIndex = 0;
...
// at a particular level. It also push all of BranchNode's ancestors to stack of parenHandles 
// from root to parent with parent on top.
// [REINSERTION]
template <int min_branch_factor, int max_branch_factor>
tree_node_handle
          ...
          Branch &branch) 
{
  ...
  for (;;) {
    ...
    // Branch Node 
    unsigned current_level = current_handle.get_level();
    ...
    // find polygon of the first branch at current Branch Node
    IsotheticPolygon node_poly = find_polygon(treeRef, current_node->entries.at(0));
    auto exp = computeExpansionArea(node_poly, insert_poly); 
    minimal_area_expansion = exp.first;
    expansions = exp.second;
    minimal_poly_area = node_poly.area(); 
    ...
    if (minimal_area_expansion != -1.0) {
      ...
      for (unsigned i = 0; i < current_node->cur_offset_; i++) {
        ...
        Branch &other_branch = current_node->entries.at(i);
        IsotheticPolygon other_poly = find_polygon(treeRef, other_branch); 
        other_poly.increaseResolution(Point::atInfinity, insert_poly);
        other_poly.refine();
      ...
      assert(insert_poly.basicRectangles.size() == expansions.size());
      for (unsigned i = 0; i < insert_poly.basicRectangles.size(); i++) {
        auto &expansion = expansions.at(i);
        auto &branch_rect = insert_poly.basicRectangles.at(i);
      ...
      // assert polygon contain polygon 
...
SplitResult BranchNode<min_branch_factor, max_branch_factor>::splitNode(
  ...
  if (parent_handle) {
    ...
    if (not is_downsplit) {
      ...
      // if we are in downsplit, it is possible left_polygon 
      // and right_polygon are empty
      // Intersect with our existing poly to avoid intersect
      // with other children
      assert(polygon_before_split.basicRectangles.size() > 0);
      // Left node: 
        ...
        bool isReinsertion)
{
  ...
  if ( givenIsPoint ) {
    ...
    for (unsigned int i = 0; i < current_node->cur_offset_; i++) {
      ...
      assert(b.boundingBox == branch_polygon.boundingBox);
      Rectangle b_mbb = branch_polygon.boundingBox;
      
      std::vector<Rectangle> &branch_rects = branch_polygon.basicRectangles;
      std::vector<Rectangle> &insertion_rects = insertion_polygon.basicRectangles;
      make_all_rects_disjoint(treeRef, branch_rects, b.child, insertion_rects, sub_branch.child);
  ...
  testDisjoint(treeRef, ret_handle, "after creating new root");
    ...
    // no need to grow the tree taller but need to update the root handle
    treeRef->root = finalSplit.rightBranch.child;
    ret_handle = finalSplit.rightBranch.child;
  } else {
    // no need to grow the tree taller and the given Point is added to entries vector
  ...
  while (!treeRef->pointQ.empty() or !treeRef->branchQ.empty()){
    ...
    if (! BranchesQueue.empty()){
      ...
      std::sort(BranchesQueue.begin(), BranchesQueue.end(), 
            [](Branch &l, Branch &r) { return l.child.get_level() > r.child.get_level(); }); 
      for (Branch &b : BranchesQueue) {
