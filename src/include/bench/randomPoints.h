...
template <> inline
std::optional<Point> PointGenerator<BenchTypeClasses::Zipf>::nextPoint(BenchTag::DistributionGenerated)
{
	if (pointBuffer.empty())
	{
...
template <> inline
std::optional<Point> PointGenerator<BenchTypeClasses::Gauss>::nextPoint(BenchTag::DistributionGenerated)
{
	// We produce all of the points at once and shove them in the buffer.
	if (pointBuffer.empty())
	{
